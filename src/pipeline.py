"""End-to-end EvidenceChain verification pipeline."""
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Run the full EvidenceChain pipeline on one or more claims."
    )
    parser.add_argument(
        "--claim",
        action="append",
        help="Claim text to verify. May be provided multiple times.",
    )
    parser.add_argument(
        "--output",
        default=os.path.join(config.RESULTS_DIR, "pipeline_test.json"),
...
if __name__ == "__main__":
    ...
    all_results = [pipeline.verify(claim) for claim in test_claims]
    ...
    for result in all_results:
        claim_short = (
            result["claim"][:42] + "..."
            if len(result["claim"]) > 42 else result["claim"]
        )
        print(
            f"{claim_short:<45} "
            f"{result['rag_verdict']:<15} "
            f"{result['bert_prob']:<8.3f} "
