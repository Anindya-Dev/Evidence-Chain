"""Retriever over the FAISS-backed EvidenceChain knowledge base."""
...
class Retriever:
    ...
    def __init__(self):
        ...
        if not os.path.exists(index_path):
            raise FileNotFoundError(
                "Knowledge-base index not found. Rebuild it with "
        ...
        if not os.path.exists(meta_path):
            raise FileNotFoundError(
                "Knowledge-base metadata not found. Rebuild it with "
                "`python -m src.rag.knowledge_base_builder`."
            )
    ...
    def retrieve(self, claim, top_k=None, use_weighting=True):
        ...
        claim_embedding = np.asarray(
            self.embedder.encode([claim]), dtype="float32"
        )
