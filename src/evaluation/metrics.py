"""Core evaluation utilities for EvidenceChain."""
...
def run_full_evaluation(pipeline, test_df, sample_size=None, text_column=None):
    ...
    if sample_size is None and config.EVAL_SAMPLE_SIZE > 0:
        sample_size = config.EVAL_SAMPLE_SIZE
        print(
            f"Explicit evaluation sample override enabled: "
            f"{sample_size} examples"
        )
    elif sample_size is None and config.CHEAP_MODE:
...
if __name__ == "__main__":
    ...
    from src.pipeline import EvidenceChain
    ...
    metrics, results, labels, preds = run_full_evaluation(
        ...
        sample_size = (
            ...
            if config.EVAL_SAMPLE_SIZE > 0 else
