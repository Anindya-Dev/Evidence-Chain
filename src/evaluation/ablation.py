"""Ablation study utilities for EvidenceChain."""
...
def run_ablation(sample_size=30):
    ...
    if sample_size == 30 and config.ABLATION_SAMPLE_SIZE > 0:
        sample_size = config.ABLATION_SAMPLE_SIZE
    elif sample_size == 30 and config.CHEAP_MODE:
...
if __name__ == "__main__":
    ...
    results = run_ablation(
        sample_size=(
            ...
            if config.ABLATION_SAMPLE_SIZE > 0 else 30
        )
    )
