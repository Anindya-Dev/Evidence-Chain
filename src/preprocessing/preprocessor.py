...
if __name__ == "__main__":
    from src.preprocessing.data_loader import load_isot, load_liar
    ...
    train_clean.to_csv(config.get_processed_split_path("train", "liar"), index=False)
    val_clean.to_csv(config.get_processed_split_path("val", "liar"), index=False)
    test_clean.to_csv(config.get_processed_split_path("test", "liar"), index=False)
    ...
    isot_train_clean.to_csv(
        config.get_processed_split_path("train", "isot"), index=False
    )
    isot_val_clean.to_csv(
        config.get_processed_split_path("val", "isot"), index=False
    )
    isot_test_clean.to_csv(
        config.get_processed_split_path("test", "isot"), index=False
    )
